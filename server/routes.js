...
export default function(genAI, upload) {
  ...
  router.post('/upload-file', upload.single('file'), async (req, res) => {
    ...
    try {
      const fileContent = await fs.readFile(req.file.path, { encoding: 'base64' });
  ...
  router.get('/conversation-messages/:conversationId', chatController.getConversationMessages());
    router.post('/generate-conversation-name', chatController.generateConversationName(genAI));
    router.post('/update-conversation-name', chatController.updateConversationName());
